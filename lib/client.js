window.__ModuleLoader__.load({
	...
	factory: function (require) {
		...
		var CSS = [
			...
			'section[role="group"] > div:first-child::after{content:attr(' + BADGE_ATTR + ');margin-left:6px;font-size:.85em;opacity:.75;font-weight:500;}',
			'section[role="group"] > div:first-child[' + CURRENT_ATTR + ']{font-weight:700;opacity:1;}',
			// 「当前」组的徽标用品牌色高亮；箭头保持 ▸/▾（折叠状态提示优先于装饰）
			'section[role="group"] > div:first-child[' + CURRENT_ATTR + ']::after{color:var(--dsw-alias-brand-primary,#4f7cff);}',
		...
		function ensurePanel() {
			...
			panelEl.id = PANEL_ID;
			// 关键修复（2026-09-12，适配 DSH 2.0.9 ModelSelect 的关闭机制）：
			// 本面板直挂 body，不在 ModelSelect 的 menu portal 内。ModelSelect 在
			// document 上监听 mousedown（closeOutside：目标不在 root/menu 内即关闭
			// 菜单），且 mousedown 把焦点移走会触发 rootRef 的 onBlur → close()。
			// 不拦这两条路径，点击面板项时 React 先卸载菜单，realBtn 变成游离节点，
			// 面板的程序化点击（dispatchEvent）再也到不了 React → 模型永远选不上。
			//   stopPropagation → 阻断 closeOutside 收到 mousedown，菜单保持打开；
			//   preventDefault  → 焦点不转移，onBlur 不触发，菜单不被误关。
